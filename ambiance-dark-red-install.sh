...
if ! grep -q "$REPO_STR" /etc/apt/*.list /etc/apt/sources.list.d/*; then
    ...
    printf "Adding Ambiance-Dark-Theme repository..."
...
if [ "$retVal" -eq 1 ]; then
	...
	if gsettings set org.gnome.desktop.wm.preferences theme "Ambiance-Dark-Red" && gsettings set org.gnome.desktop.interface gtk-theme "Ambiance-Dark-Red"; then
	    ...
	    printf "Error enabling theme. Please run \"gsettings set org.gnome.desktop.wm.preferences theme 'Ambiance-Dark-Red' && gsettings set org.gnome.desktop.interface gtk-theme 'Ambiance-Dark-Red'\" manually."
	    exit 1
	fi
    else
	printf "Error installing ambiance-dark-red theme. Error: \n%s\n" "$msg"
	exit 1
    fi
else
